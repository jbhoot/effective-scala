...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
    ...
    val size = sortedGrades.size
    // no need to check for odd or even
    // because in case of even size,
    // I am supposed to pick any of the two middle elements.
    // evenSize / 2 will always give the index of the latter of the two,
    // which is fine by me.
...
case class Election(description: String, candidates: Set[Candidate]):
  ...
  def elect(ballots: Seq[Ballot]): Candidate =
    ...
    val allGrades: Seq[(Candidate, Grade)] =
      ballots.flatMap(b => b.grades.toSeq)
  ...
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      val bestMedianGrade: Grade =
        gradesPerCandidate.values.filter(_.nonEmpty).map(median).maxBy(_.ordinal)
      ...
      val bestCandidates: Map[Candidate, Seq[Grade]] =
        gradesPerCandidate.filter((candidate, grades) => median(grades) == bestMedianGrade)
