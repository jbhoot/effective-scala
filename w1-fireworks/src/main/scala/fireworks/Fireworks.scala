...
case class Waiting(countDown: Int, startPosition: Point, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      ...
      Launched.init(position = startPosition, numberOfParticles, particlesColor)
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      val newPosition = Motion.movePoint(position, direction, speed = Settings.propulsionSpeed)
      copy(countDown = countDown - 1, position = newPosition)
    else
      Exploding.init(numberOfParticles, direction, position, color = particlesColor)
